...
int ret = -1;
jmethodID deleteMethod = nullptr;
jboolean deleted;
FILE* finalOutputStream;
FILE* tempFileRead;
jmethodID getAbsolutePathMethod;
jstring tempOutputFilePathJStr;
const char *tempOutputFilePath;
jstring tempInputFileUriJStr;
jobject tempOutputFileObj;
jclass fileClass;
jmethodID createTempFileMethod;
jstring prefix;
jstring suffix;
...
createTempFileMethod = env->GetStaticMethodID(fileClass, "createTempFile", "(Ljava/lang/String;Ljava/lang/String;Ljava/io/File;)Ljava/io/File;");
prefix = env->NewStringUTF("decoded_");
suffix = env->NewStringUTF(".yuv");

// Get cache directory from context to store temp file
getCacheDirMethod = env->GetMethodID(env->GetObjectClass(context), "getCacheDir", "()Ljava/io/File;");
cacheDirFile = env->CallObjectMethod(context, getCacheDirMethod);
...
tempOutputFileObj = env->CallStaticObjectMethod(fileClass, createTempFileMethod, prefix, suffix, cacheDirFile);
...
tempOutputFilePathJStr = (jstring) env->CallObjectMethod(tempOutputFileObj, getAbsolutePathMethod);
tempOutputFilePath = env->GetStringUTFChars(tempOutputFilePathJStr, nullptr);
...
tempInputFileUriJStr = env->NewStringUTF(inputFileUriStr);
...
tempFileRead = fopen(tempOutputFilePath, "rb");
if (!tempFileRead) {
...
finalOutputStream = fdopen(outputFd, "wb"); // outputFd is from get_fd_from_uri (which came from ParcelFileDescriptor)
...
deleteMethod = env->GetMethodID(fileClass, "delete", "()Z");
deleted = env->CallBooleanMethod(tempOutputFileObj, deleteMethod);
if (!deleted) {
